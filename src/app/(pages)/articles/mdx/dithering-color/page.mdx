...
import DemoPatternImage from '../introduction-to-dithering/DemoPatternImage';
import DemoOrdered from '../introduction-to-dithering/DemoOrdered';
import ImageGrid from '../introduction-to-dithering/ImageGrid';
...
export const metadata = {
  title: 'Introduction to dithering in color — Teo Fumagalli',
  description:
    'This follow-up article explains how dithering can be made to work with arbitrary color palettes.',
};
...
In short, the human eye has a _non-linear response_ to light: we're better at perceiving small changes in brightness in darker places than overall brighter ones. What this means is we don't see twice as much light as twice the brightness: what we see as a “50% gray” actually reflects only about 20% of incoming light.

Monitors benefit from encoding color in the same non-linear way: brightness becomes _perceptually uniform_, which makes working with color much easier, and out of all the possible brightness levels (256 for most screens), more are dedicated to the darker tones where we can better spot small changes in brightness.
...
The exponent of this power function is called _gamma_. Most screens use a gamma value between 2.2 and 2.4, which gives an output of about 20% brightness for an input of 50%.
...
Let's take a closer look at the `findClosestFromPalette` function. For black and white, as we've seen, a simple threshold check will do, but that isn't the case for any palette. We'll need a way to find the closest color from a list, and that means a way to find the _distance_ between colors.
...
<ImageGrid
  minSize="30ch"
  pixelated
  basePath="/Documents/Articles/assets/dither2/"
  images={[
    {
      src: 'car-original',
      formats: ['webp', 'jpeg'],
...
This is an expanded version of a dithering algorithm named _pattern dithering_, so I'm using that name. It's the algorithm used by Adobe Photoshop to dither images when saving to a format with a limited color palette, like a GIF or indexed-color PNG.
...
This method is more complex, and far more elegant, than the last one. Instead of looking for thepair of colors that make the best mix, we're going to find the best mix including _any_ number of colors.
