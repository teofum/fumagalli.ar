...
import softwareRenderProcess from '../dither/software';
import { getPaletteSize } from '../dither/utils/palette-colors';
import RenderWorker, { type ImagePart } from '../dither/software/render-worker';
...
export default function useSoftwareRenderer(rt: HTMLCanvasElement | null) {
  const [state] = useAppState('dither');
  const image = useImage();

  const process = state.process as keyof typeof softwareRenderProcess;
  const { palette, settings } = state;
  ...
  const render = useCallback(async () => {
    ...
    ctxTemp.drawImage(bmp, 0, 0, rt.width, rt.height);
  ...
  }, [image, rt, ctx, palette, process, settings]);
