...
In technical terms, _dither_ is noise introduced to a signal to hide unwanted large-scale patterns. For an image, these are artifacts like the color banding seen when using a limited color palette. In other words, it's a way to trick the eye into seeing more colors than are actually being displayed. This is achieved by mixing colors together in small, repeating patterns.
